...
export class LoadComponent extends BaseView {
    ...
    protected addedHandler() {
        if (!this.isInit) {
            GRoot.inst.showModalWait(this.getString(LibStr.LOADING))
            MyLoader.loader.load(this.loadArray,
    ...
    private progressHandler(data: number) {
        GRoot.inst.showModalWait(this.getString(LibStr.LOADING) + " " + Math.floor(data * 100) + "%")
    }
