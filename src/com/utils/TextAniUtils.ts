...
export class TextAniUtils {
    ...
    private _playClean(playText: string = null) {
        ...
        this.playTwinkle(2, Handler.create(this, (playText: string) => {
            ...
            for (let i = 0; i < showTextLen; i++) {
                ...
                Tween.to(charData, {
                    count: 0,
                    update: new Handler(this, this.changeTextHandler, [charData, this._playText.charAt(i)])
                }, 300, null, Handler.create(this, this.cleanTextEndHandler, [playText]), 300)
            }
        }, [playText]))
    }
    ...
    /** 清除结束 */
    private cleanTextEndHandler(playText: string) {
        this.clearCount++
...
//        console.log("默认文本 = " + this.aniText, "len = " + this.aniText.length)
        ...
        for (let i = 0; i < showTextLen; i++) {
            ...
            Tween.to(charData, {
                count: start + i,
                update: new Handler(this, this.changeTextHandler, [charData, this._playText.charAt(i)])
            }, 200, null, Handler.create(this, this.changeTextEndHandler), 15 * i)
        }
    }
    ...
    private changeTextHandler(charData: any, txt: string) {
