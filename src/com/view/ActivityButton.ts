...
export class ActivityButton extends BaseButton {
    ...
    setCorner(value: number) {
        ...
        if (value > 0) {
            this.contentText.text = StringUtil.format(this.getString(LibStr.USE_IN_GIFT), value)
        } else {
            this.contentText.text = this.getString(LibStr.NOT_GIFT)
        }
    ...
    openDrag() {
        ...
        let arr: any[] = LocalStorage.getJSON("activity_" + Player.inst.gameModel)
        if (arr != null) {
